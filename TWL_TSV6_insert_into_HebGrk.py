...
def read_USFM_file(BBB:str, nn:str) -> Tuple[Path,List[str]]:
    ...
    """
    global ol_path, twl_path
    source_filename = f'{nn}-{BBB}.usfm'
    source_filepath = os.path.join(ol_path, source_filename)
...
def read_TWL_file(BBB:str) -> List[List[str]]:
    """
    Function to read the TWL 6-column TSV file for a given book (BBB)
    """
    global twl_path
    input_filepath = os.path.join(twl_path, f'twl_{BBB}.tsv')
...
def handle_book(BBB:str, nn:str) -> Tuple[int,int]:
    ...
    all_TWL_source_lines = read_TWL_file(BBB)
...
def insert_twl_into_ol(ol, twl):
    ...
    print(f"  TWL source folderpath is {twl_path}")
    print(f"  OrigL folderpath is {ol_path}")
    if ol_path.split(os.path.sep)[-1] == 'el-x-koine_ugnt':
        nt = True
    total_simple_links = total_complex_links = 0
    fail_list = []
    for BBB,nn in BOOK_NUMBERS.items(): # This script just handles exactly 66 books
        print(BBB, nn)
        if nn < '01' or nn > '67':
            continue
        if nn < '40' and nt:
            continue # Skip OT
        elif nn > '40' and not nt:
            continue # Skip NT
        try:
            simple_count, complex_count = handle_book(BBB.upper(), nn)
            total_simple_links += simple_count
            total_complex_links += complex_count
        except Exception as e:
            fail_list.append(BBB.upper())
            print(f"ERROR: failed to process {BBB.upper()}: {e}")
    if fail_list:
        ...
        print(f"    {total_simple_links+total_complex_links:,} total links written ({total_simple_links:,} simple links and {total_complex_links:,} multiword links) to {ol_path}")
# end of main function

if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python TWL_TSV_insert_into_HebGrk.py <path to greek or hebrew bible> <path to twl repo>")
        sys.exit(1)
    if not os.path.exists(sys.argv[1]):
        print(f"OL Bible path does not exist: {sys.argv[1]}")
        sys.exit(1)
    if not os.path.exists(sys.argv[2]):
        print(f"TWL path does not exist: {sys.argv[2]}")
        sys.exit(1)
    if sys.argv[1].split(os.path.sep)[-1] not in ['el-x-koine_ugnt', 'hbo_uhb']:
        print("OL Bible path must be of hbo_uhb or el-x-koine_ugnt")
    if sys.argv[2].split(os.path.sep)[-1].split('_')[-1] != 'twl':
        print("TWL path must be a TWL repo directory")
    insert_twl_into_ol(sys.argv[1], sys.argv[2])
