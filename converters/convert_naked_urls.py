...
def fix_naked_urls(given_html):
    ...
    """
    # AppSettings.logger.debug(f"fix_naked_urls( ({len(given_html)}) {repr(given_html) if len(given_html)<200 else repr(given_html)[:200]+'…'})…")
    ...
    for j, regex in enumerate((r'<((?:ftp|http|https)://(?:[\.\w\d]+?)\.(?:com|org|net|us|bible)(?:[/=\?\w\d]+))>',
        ...
        while True:
            ...
            if not match: break
            AppSettings.logger.debug(f"Found naked URL matchA{j}: '{match.group(1)}' inside '{match.group(0)}'")
    ...
    for j, regex in enumerate((r'<((?:[\.\w\d]+?)\.(?:com|org|net|us|bible)(?:[/=\?\w\d]+))>',
        ...
        while True:
            match = re.search(regex, result_html, re_flags)
            if not match: break
            AppSettings.logger.debug(f"Found naked URL matchC{j}: '{match.group(1)}' inside '{match.group(0)}'")
    ...
    for j, regex in enumerate((r'(?:^|[ \(\[])((?:[\.\w\d]+?)\.(?:com|org|net|us|bible)(?:$|[/=\?\w\d]+))',
        ...
        while True:
            match = re.search(regex, result_html, re_flags)
            if not match: break
            AppSettings.logger.debug(f"Found naked URL matchD{j}: '{match.group(1)}' inside '{match.group(0)}'")
    ...
    while True:
        ...
        if not match: break
        AppSettings.logger.debug(f"Found naked email matchE: '{match.group(1)}' inside '{match.group(0)}'")
    ...
    while True:
        ...
        if not match: break
        AppSettings.logger.debug(f"Found naked email matchF: '{match.group(1)}' inside '{match.group(0)}'")
