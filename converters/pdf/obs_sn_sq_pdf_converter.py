...
class ObsSnSqPdfConverter(PdfConverter):
    ...
    @property
    def title(self):
        sn_title = self.resources[f'{self.lang_code}_obs-sn'].title
        sq_title = self.resources[f'{self.lang_code}_obs-sq'].title
        return f'{sn_title}\n<br/>\n&\n<br/>\n{sq_title}'

    @property
    def simple_title(self):
        sn_title = self.resources[f'{self.lang_code}_obs-sn'].simple_title
        sq_title = self.resources[f'{self.lang_code}_obs-sq'].simple_title
...
<section id="{self.lang_code}-obs-sn">
    <div class="resource-title-page no-header">
        <img src="{self.resources[f'{self.lang_code}_obs-sn'].logo_url}" class="logo" alt="OBS">
        <h1 class="section-header">{self.simple_title}</h1>
    </div>
'''
        intro_file = os.path.join(self.resources[f'{self.lang_code}_obs-sq'].repo_dir, 'content', '00.md')
...
'''
        for chapter_num in range(1, 51):
            ...
            sn_chapter_dir = os.path.join(self.resources[f'{self.lang_code}_obs-sn'].repo_dir, 'content', chapter_num)
            sq_chapter_file = os.path.join(self.resources[f'{self.lang_code}_obs-sq'].repo_dir, 'content', f'{chapter_num}.md')
