...
'''
    ...
    def add_rc(self, rc_link, article='', title=None, linking_level=0, article_id=None):
        rc = self.create_rc(rc_link, article=article, title=title,
                            linking_level=linking_level, article_id=article_id)
        self.rcs[rc.rc_link] = rc
        return rc

    def add_appendix_rc(self, rc_link, article='', title=None, linking_level=0):
        rc = self.create_rc(rc_link, article=article,
                            title=title, linking_level=linking_level)
    ...
    def setup_style_sheets(self):
        ...
        for style_file in self.style_sheets:
            ...
            shutil.copy(os.path.join(self.pdf_converters_dir,
...
'''
    ...
    def setup_resources(self):
        ...
        if self.my_subject == OPEN_BIBLE_STORIES and not os.path.exists(os.path.join(self.source_dir, 'content')):
            # Use the massaged OBS dir from door43-job-handler to handle tS repos as well
        ...
        resource = Resource(subject=self.my_subject, repo_name=self.repo_name, owner=self.owner, ref=self.ref,
                            zipball_url=zipball_url, repo_dir=repo_dir)
        ...
        for subject in REQUIRED_RESOURCES[self.my_subject]:
            ...
            # if not resource:
            #     resource = self.process_relation_resource(self.language_id + "/" + SUBJECT_ALIASES[subject][0])
            #     print("WE GO TTHIS:")
            #     print(resource)
            if resource:
                self.resources[resource.identifier] = resource
...
'''
            ...
            for part in parts[1:]:
                if not re.search(rc_regex, part):
                    ...
                    if rc_link in self.all_rcs:
                        if self.all_rcs[rc_link].linking_level <= APPENDIX_LINKING_LEVEL:
                            ...
                            part = BeautifulSoup(f'<span class="remove-article">{self.all_rcs[rc_link].title} <a href="https://git.door43.org/attachments/90c6a9af-c89b-445a-be9d-b8de92880d40" target="ta-pdf">(UTA PDF)</span>', 'html.parser').find('span')
                    else:
                        part = soup.new_string(part)
    ...
    def crawl_ta_tw_deep_linking(self, source_rc: ResourceContainerLink):
        ...
        for rc_link in rc_links:
            ...
            if not rc.article and not already_crawled:
    ...
    def get_appendix_html(self, resource):
        ...
        for item in sorted_rcs:
            rc = item[1]
            if rc.article:
                html += rc.article.replace('</article>',
                                           self.get_referenced_in_html(rc) + '</article>')
        if html:
            html = f'''
<section>
    ...
    def get_referenced_in_html(self, source_rc):
        ...
        for rc_link in source_rc.references:
            if rc_link in self.rcs:
                rc = self.rcs[rc_link]
                referenced_in = re.sub("(^|:|-)0", r"\1", rc.title)
                referenced_ins.append(
                    f'<a href="#{rc.article_id}">{referenced_in}</a>')
        referenced_in_html = ''
...
'''
    ...
    def get_catalog_entry(self, resource_name, owners, langs, refs):
        for owner in owners:
            for lang in langs:
    ...
    def process_relation_resource(self, relation):
        ...
        ref = DEFAULT_REF
        langs_to_try = [lang]
        owners_to_try = [self.owner]
        for owner in OWNERS:
            if owner not in owners_to_try:
                owners_to_try.append(owner)
        refs_to_try = [ref]
        if '/' in relation:
            lang, resource_name = relation.split('/')[0:2]
            if lang not in langs_to_try:
                langs_to_try.insert(0, lang)
        else:
            resource_name = relation
        if '?' in resource_name:
            resource_name, ref = resource_name.split('?')[0:2]
            ref = ref.replace('=', '')
            # We do not use the given ref, but only master, if the main resource we are generating for is the master branch
            if self.main_resource.ref != DEFAULT_REF:
                if ref not in refs_to_try:
                    refs_to_try.insert(0, ref)
                if ref.startswith("v") and ref[1:] not in refs_to_try:
                    refs_to_try.insert(1, ref[1:])
        if (resource_name == "ugnt" or resource_name == "uhb") and self.owner != "unfoldingWord":
            owners_to_try.insert(0, "unfoldingWord")
        ...
        if self.debug_mode and os.path.exists(repo_dir):
            resource = Resource(
                owner=self.owner, repo_name=repo_name, repo_dir=repo_dir, ref=ref)
            self.relation_resources[resource.identifier] = resource
            return

        entry = self.get_catalog_entry(
            resource_name, owners_to_try, langs_to_try, refs_to_try)
        if not entry:
    ...
    def find_catalog_entries(self, subject, lang=None):
        ...
        if subject == GREEK_NEW_TESTAMENT:
            stage = STAGE_PROD
            owner = 'unfoldingWord'  # Have to hardcode this since this owner has aligned TW data
        ...
        for keyed_args in keyed_args_to_find_best_match:
            try:
                response = AppSettings.catalog_api.catalog_search(
                    subject=subject, sort='released', order='desc', **keyed_args)
    ...
    def download_resource(self, resource):
        self.log.info(
            f"download_source_file( {resource.zipball_url}, {self.download_dir} )")
        source_filepath = os.path.join(
            self.download_dir, resource.repo_name + '.zip')
        if resource.repo_dir:
            repo_dir = resource.repo_dir
        else:
            repo_dir = os.path.join(self.download_dir, resource.repo_name)
        self.log.info(
            f"source_filepath: {source_filepath}, repo_dir: {repo_dir}")
        if not os.path.exists(repo_dir):
            ...
            try:
                ...
                if os.path.exists(repo_dir):
                    ...
                    self.log.error(
                        f"Unable to find repo directory after unzipping {resource.zipball_url}")
        else:
            resource.repo_dir = repo_dir
