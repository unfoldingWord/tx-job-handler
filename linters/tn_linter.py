...
class TnLinter(MarkdownLinter):
    ...
    def __init__(self, single_file=None, *args, **kwargs):
        ...
        GlobalSettings.logger.debug("Convert single '{0}'".format(self.single_file))
        self.single_dir = None
        if self.single_file:
            parts = os.path.splitext(self.single_file)
            self.single_dir = self.get_dir_for_book(parts[0])
            GlobalSettings.logger.debug("Single source dir '{0}'".format(self.single_dir))
    ...
    def lint(self):
        ...
        for dir in BOOK_NUMBERS:
            ...
            if self.single_dir and (dir != self.single_dir):
                continue
            GlobalSettings.logger.debug("Processing folder {0}".format(dir))
        ...
        if not results:
            GlobalSettings.logger.debug("Error running MD linter on {0}".format(self.s3_results_key))
        return results
